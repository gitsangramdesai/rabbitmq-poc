...
amqp.connect(conString, function (err, conn) {
    conn.createConfirmChannel(function (err, channel) {
        ...
        for (var i = 0; i < 20; i++) {
            ...
            var channelOption = {};
            channel.sendToQueue(queue, buffered_msg, channelOption, function (err, ok) {
                if (err !== null){
                    console.warn('Message nacked :' + msg.sr_no.toString());
                }                   
                else{
                    console.log('Message acked :' + + msg.sr_no.toString());
                }
            });
        }
    });
